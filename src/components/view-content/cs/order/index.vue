        ...
        <template #content>
          <div>
            ...
            >
              ...
              <Column style="min-width: 150px">
                <template #header>
                  <span class="text-sm font-bold">{{ t('creator') }}</span>
                </template>
                <template #body="slotProps">
                  <span class="text-sm text-color">{{ slotProps.data.user.name || '' }}</span>
                </template>
              </Column>
...
const resetFilter = () => {
  ...
  queryParams.value.customer = null
  dates.value = []
...
const getOrders = async () => {
  try {
    const res = await api.get('cs/get-orders', {
...
const getCustumers = async () => {
  try {
    const res = await api.get('cs/get-customers')
